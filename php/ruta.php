...
$sql_mant = mysqli_query($conn,"SELECT tmp_reportes.id_reporte FROM tmp_reportes INNER JOIN reportes ON tmp_reportes.id_reporte = reportes.id_reporte WHERE (tmp_reportes.ruta = $Id AND reportes.descripcion LIKE 'Mantenimiento:%')");
...
$sql_orden = mysqli_query($conn,"SELECT id_reporte FROM tmp_reportes WHERE (ruta = $Id AND id_reporte > 100000)");
...
$sql_pedidos_r = mysqli_query($conn,"SELECT * FROM pedidos WHERE id_orden = $Id");
...
if (count($Pedidos) > 0) {
    foreach ($Pedidos as $clave => $valor) {
        ...
        if ($rep_ruta['bobina'] == 1 AND $rep_ruta['vale'] == 1){
            $this->MultiCell(155,10,utf8_decode($ListPedidos.$MATERIAL.'1.- OTORGAR BOBINA NUEVA PARA LA RUTA ('.$Id.') A NOMBRE DE: '.$ruta['responsable'].'
...
RESPONSABLE DE RUTA: '.$ruta['responsable']),1,'C',false);
            ...
            $this->MultiCell(155,10,utf8_decode($ListPedidos.$MATERIAL.'1.- OTORGAR VALE DE GASOLINA PARA LA RUTA ('.$Id.') EN EL VEHICULO: '.$rep_ruta['vehiculo'].'
  _____________________________
RESPONSABLE DE RUTA: '.$ruta['responsable']),1,'C',false);
        }else{
