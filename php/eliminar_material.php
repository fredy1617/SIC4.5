...
$material=mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM stock_tecnicos WHERE serie = $Serie"));
...
if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM historial_stock WHERE serie = $Serie AND material = '$nombre'"))>0){
	...
	if(mysqli_query($conn, "DELETE FROM stock_tecnicos WHERE serie = '$Serie'")){
