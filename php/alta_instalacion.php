		       ...
		       $ARRAY = $API->parse_response($READ); 
		        if(count($ARRAY)>0){ // si el nombre de usuario "ya existe" lo edito
		        	...
		        	if(mysqli_query($conn,$sql)){
						...
						if ($Liquidar != 0) {	
							$sql = "INSERT INTO pagos (id_cliente, descripcion, cantidad, fecha, hora, tipo, id_user, corte, tipo_cambio) VALUES ('$IdCliente', '$Descripcion', '$Liquidar', '$FechaInstalacion', '$Hora', '$Tipo_t', '$id_user', 0, '$Tipo_Campio')";
							if ($Tipo_Campio == "Credito") {
							  ...
							  $id_deuda = $ultimo['id'];
							  $sql = "INSERT INTO pagos (id_cliente, descripcion, cantidad, fecha, hora, tipo, id_user, corte, tipo_cambio, id_deuda) VALUES ('$IdCliente', '$Descripcion', '$Liquidar', '$FechaInstalacion', '$Hora', '$Tipo_t', '$id_user', 0, '$Tipo_Campio', $id_deuda)";
							}
							if(mysqli_query($conn, $sql)){
